...
export function MachineAttachments({ machineId, attachments: initial, canEdit }: Props) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch(`/api/machines/${machineId}/attachments`, {
      method: "POST",
      body: formData,
    })
  ...
  async function handleDelete(attachmentId: string, fileName: string) {
    ...
    const res = await fetch(`/api/machines/${machineId}/attachments/${attachmentId}`, {
      method: "DELETE",
    })
    ...
    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
      ...
      {attachments.length === 0 ? (
        <p className="px-5 py-6 text-sm text-slate-400 text-center">
          Aucune pièce jointe — photos, PDF, fiches techniques…
        </p>
      ) : (
        <ul className="divide-y divide-slate-100">
          {attachments.map((a) => {
            const isImage = a.mimeType.startsWith("image/")
            const url = `/uploads/machines/${machineId}/${a.storedName}`
